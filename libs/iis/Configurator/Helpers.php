...
final class Helpers
{
	...
	public static function detectDebugMode(array $allowedIpAddresses = []): bool
	{
		...
		if (
			!isset($_COOKIE[\IIS\Configurator\Configurator::SECRET_DEBUG_NAME])
			|| !(bool) $_COOKIE[\IIS\Configurator\Configurator::SECRET_DEBUG_NAME]
		) {
			return false;
		}

		if (\getenv(\IIS\Configurator\Configurator::SECRET_DEBUG_NAME) !== false) {
			return (bool) \getenv(\IIS\Configurator\Configurator::SECRET_DEBUG_NAME);
